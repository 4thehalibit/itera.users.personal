...
// LOCAL FORK of itera pkgs/dms-itera-update, wired in by hosts/common.nix
// (`plugins.iteraUpdate.src`, an mkDefault upstream). The only change is the
// button row at the bottom of the popout: upstream is deliberately read-only
// (see the note above), which leaves reading a command out of a popup and
// retyping it in a terminal as the last manual step. Re-copy this directory
// from the itera source whenever the pill itself gains anything upstream.
import QtQuick
import Quickshell
...
PluginComponent {
    ...
    // A terminal, not a detached process: everything these buttons run ends in a
    // privileged rebuild, and `sudo` with no tty has nowhere to ask for the
    // password. The trailing read keeps the window up on failure, which is the
    // run you actually need to see.
    function runInTerminal(command) {
        Quickshell.execDetached(["wezterm", "start", "--class", "itera-update", "--", "bash", "-lc", "cd \"$HOME/Documents/itera.users.personal\" && " + command + "; echo; read -rp 'Press enter to close...'"]);
    }
    ...
    popoutHeight: Math.min(560, 274 + Math.min(root.commits.length, 10) * 34)
                    ...
                    Column {
                        ...
                        Row {
                            ...
                            DankButton {
                                ...
                                onClicked: root.runInTerminal("nix flake update itera --refresh && deploy 'chore: bump itera'")
                            }
                            ...
                            DankButton {
                                ...
                                buttonHeight: 32
                                onClicked: root.runInTerminal("itera update-boot")
                            }
                        }
                    }
