...
#
# No auto-restore: back up whatever you need (Documents, .claude, Wi-Fi
# profiles) BEFORE wiping, and restore it manually after first boot.
#
...
#   curl -sSL .../install.sh | sudo HOST=generic bash              # emergency spare hardware
#   curl -sSL .../install.sh | sudo HOST=personal bash             # no corp modules
...
HOST="${HOST:-framework}"     # nixosConfigurations attr: framework (default), generic, or personal
...
echo "Restore any backed-up Documents/.claude/Wi-Fi manually — no auto-restore."
